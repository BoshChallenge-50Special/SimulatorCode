...
class StateMachineVelocity:
    ...
    def Slow_transitions(self, data):
        if self.conditionOr("==", data, ["stop_signal", "pedestrian"], [True, True]):
            newState = "OnSteady"
        elif self.conditionAnd("==", data, ["horizon_line", "pedestrian_line", "state_steer"], [False, False, "OnLane"]):
            newState = "Fast"
        else:
            newState = "Slow"
        return newState
    ...
    def Fast_transitions(self, data):
        if self.conditionOr("==", data, ["horizon_line"], [True]):
            newState = "Slow"
        else:
            newState = "Fast"
        return newState
